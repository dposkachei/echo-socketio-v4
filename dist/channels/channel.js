...
var Channel = (function () {
    ...
    Channel.prototype.leave = function (socket, channel, reason) {
        if (channel) {
            ...
            if (this.options.devMode) {
                log_1.Log.info("[".concat(new Date().toISOString(), "] - ").concat(socket.id, " left channel: ").concat(channel, " (").concat(reason, ")"));
            }
        }
    };
    ...
    Channel.prototype.onJoin = function (socket, channel) {
        if (this.options.devMode) {
            log_1.Log.info("[".concat(new Date().toISOString(), "] - ").concat(socket.id, " joined channel: ").concat(channel));
        }
    };
